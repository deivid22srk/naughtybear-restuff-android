...
void vt_handle_vkCreateInstance(VkContext* context) {
    ...
    // Port Android: no upstream TODAS as extensões de surface eram removidas
    // (o swapchain X11 era sintético). No modo android-surface o host precisa
    // de VK_KHR_surface + VK_KHR_android_surface habilitados na instance real
    // para criar a superfície/swapchain — só VK_KHR_xlib_surface sai fora.
    const char* skipExtensions[] = {"VK_KHR_xlib_surface"};
...
void vt_handle_vkEnumerateInstanceExtensionProperties(VkContext* context) {
    ...
    // Port Android: o host Android (loader + driver) já enumera
    // VK_KHR_surface e VK_KHR_android_surface nativamente — não escondemos
    // mais a extensão android (upstream a removia porque o swapchain X11 era
    // sintético). Extras vazios: nada a injetar; skip apenas defensivo do
    // xlib (não enumerado no Android de qualquer forma).
    const char* extraExtensions[] = {"VK_KHR_surface"};
    const char* skipExtensions[] = {"VK_KHR_xlib_surface"};
...
// ---------------------------------------------------------------------------
// Port Android (naughtybear-restuff-android): modo android-surface passthrough
// ---------------------------------------------------------------------------
// No upstream (Winlator/Wine) a VkSurfaceKHR do cliente é um id de janela X11:
// o servidor FABRICA capabilities/formats/present modes e o swapchain é um
// XWindowSwapchain (imagens AHardwareBuffer + JNI no XServer). Neste port a
// superfície é criada DE VERDADE no driver do host pelo handler
// vt_handle_vkCreateAndroidSurfaceKHR (abaixo) e o id da superfície É o handle
// real — logo todos estes handlers fazem a consulta/criação REAL no driver.
// ---------------------------------------------------------------------------

void vt_handle_vkCreateAndroidSurfaceKHR(VkContext* context) {
    uint64_t instanceId;
    uint64_t window;
    VkAndroidSurfaceCreateFlagsKHR flags;

    vt_unserialize_vkCreateAndroidSurfaceKHR((VkInstance)&instanceId, &window, &flags, context->inputBuffer, &context->memoryPool);
    VkInstance instance = VkObject_fromId(instanceId);

    // Cliente e servidor vivem no MESMO processo: o ANativeWindow* enviado
    // pelo cliente é diretamente válido aqui.
    ...
    if (vulkanWrapper.vkCreateAndroidSurface) {
        result = vulkanWrapper.vkCreateAndroidSurface(instance, &createInfo, NULL, &surface);
    }
    if (result == VK_ERROR_DEVICE_LOST) context->status = result;

    // O id da superfície enviado ao cliente É o handle real do host.
    VT_SERIALIZE_CMD(VkSurfaceKHR, surface);
    vt_send(context->clientRing, result, outputBuffer, bufferSize);
}

void vt_handle_vkDestroySurfaceKHR(VkContext* context) {
    uint64_t instanceId;
    uint64_t surfaceId;

    vt_unserialize_vkDestroySurfaceKHR((VkInstance)&instanceId, (VkSurfaceKHR)&surfaceId, NULL, context->inputBuffer, &context->memoryPool);
    VkInstance instance = VkObject_fromId(instanceId);
    VkSurfaceKHR surface = VkObject_fromId(surfaceId);

    if (vulkanWrapper.vkDestroySurface) vulkanWrapper.vkDestroySurface(instance, surface, NULL);
}
...
void vt_handle_vkGetPhysicalDeviceSurfaceFormatsKHR(VkContext* context) {
    ...
    VkSurfaceFormatKHR* surfaceFormats = surfaceFormatCount > 0 ? vt_alloc(&context->memoryPool, surfaceFormatCount * sizeof(VkSurfaceFormatKHR)) : NULL;
    VkResult result = vulkanWrapper.vkGetPhysicalDeviceSurfaceFormats(physicalDevice, surface, &surfaceFormatCount, surfaceFormats);
    if (result == VK_ERROR_DEVICE_LOST) context->status = result;
...
void vt_handle_vkQueuePresentKHR(VkContext* context) {
    ...
    // Port Android: payload estendido — (queue, presentInfo). O queue é
    // necessário para o vkQueuePresentKHR real do host. Present segue
    // FIRE-AND-FORGET no protocolo (sem resposta — upstream idem), com
    // DEVICE_LOST propagado via context->status.
    vt_unserialize_vkQueuePresentKHR((VkQueue)&queueId, &presentInfo, context->inputBuffer, &context->memoryPool);
    VkQueue queue = VkObject_fromId(queueId);
...
void vt_handle_vkAcquireNextImage2KHR(VkContext* context) {
    ...
    vt_unserialize_vkAcquireNextImage2KHR((VkDevice)&deviceId, &acquireInfo, NULL, context->inputBuffer, &context->memoryPool);
    VkDevice device = VkObject_fromId(deviceId);

    // Port Android: passthrough real — swapchain/semaphore/fence do
...
HandleRequestFunc handleRequestFuncs[] = {
    ...
    // Port Android: entradas NOVAS no final — alinhadas com os códigos 354/355
    // adicionados ao final de request_codes.h (a ordem do array é posicional).
    vt_handle_vkCreateAndroidSurfaceKHR,
    vt_handle_vkDestroySurfaceKHR,
};
