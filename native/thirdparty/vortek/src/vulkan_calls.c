...
// Port Android: exposto para os wrappers padrão do loader em main.c.
void* findVkDispatchFuncWithNamePublic(const char* name) {
    return findVkDispatchFuncWithName(name);
}
...
VkResult vt_call_vkWaitForFences(VkDevice device, uint32_t fenceCount, const VkFence* pFences, VkBool32 waitAll, uint64_t timeout) {
    ...
    if (timeout == 0) {
        ...
        // numFds==0 + status = espera concluída no SERVIDOR (fallback do
...
// Port Android (naughtybear-restuff-android): no upstream (modo Winlator/X11)
// a VkSurfaceKHR do cliente é um objeto PURAMENTE local cujo id é o id da
// janela X11 — o servidor fabrica capabilities/formats e o swapchain é um
// XWindowSwapchain (AHardwareBuffer + JNI). Neste port (android-surface
// passthrough) a superfície é CRIADA DE VERDADE no driver do host pelo
// servidor: round-trip com o ponteiro do ANativeWindow (mesmo processo, logo
// o ponteiro é válido dos dois lados) e o id devolvido É o handle real da
// VkSurfaceKHR do host — todos os handlers de surface/swapchain do servidor
// fazem cast direto (mesma convenção de ids dos demais objetos).
VkResult vt_call_vkCreateAndroidSurfaceKHR(VkInstance instance, const VkAndroidSurfaceCreateInfoKHR* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSurfaceKHR* pSurface) {
    VT_CALL_LOCK();
    VkObject* instanceObject = VkObject_fromHandle(instance);

    VT_SERIALIZE_CMD(vkCreateAndroidSurfaceKHR, (VkInstance)&instanceObject->id, (uint64_t)(uintptr_t)pCreateInfo->window, pCreateInfo->flags);
    VT_SEND_CHECKED(REQUEST_CODE_VK_CREATE_ANDROID_SURFACE_KHR, VT_RETURN);
    VT_RECV_CHECKED(VT_RETURN);

    uint64_t surfaceId;
    vt_unserialize_VkSurfaceKHR((VkSurfaceKHR)&surfaceId, inputBuffer, &globalMemoryPool);
    VkObject* surfaceObject = VkObject_create(VK_OBJECT_TYPE_SURFACE_KHR, surfaceId);
...
// Port Android: a destruição agora é encaminhada ao servidor para destruir a
// VkSurfaceKHR real do host (no upstream era um no-op local — as superfícies
// X11 não existiam no servidor).
void vt_call_vkDestroySurfaceKHR(VkInstance instance, VkSurfaceKHR surface, const VkAllocationCallbacks* pAllocator) {
    ...
    VT_SERIALIZE_CMD(vkDestroySurfaceKHR, (VkInstance)&instanceObject->id, (VkSurfaceKHR)&surfaceObject->id, NULL);
    vt_send(serverRing, REQUEST_CODE_VK_DESTROY_SURFACE_KHR, outputBuffer, bufferSize);
...
VkResult vt_call_vkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo) {
 ...
 // Port Android: o queue agora também é serializado — o servidor precisa dele
 // para chamar o vkQueuePresentKHR real do host no modo android-surface
 // (no modo X11 upstream o present ia para XWindowSwapchain_presentImage e o
 // queue era irrelevante). O present permanece FIRE-AND-FORGET como no
 // upstream: o servidor NÃO envia resposta (ver vt_handle_vkQueuePresentKHR)
 // e o cliente retorna imediatamente — esperar resposta aqui causaria
 // deadlock no protocolo.
...
// Port Android: funções Xlib só existem quando os headers X11 estão
// disponíveis (build do rootfs glibc do upstream). No NDK não há X11 — o
// caminho Xlib é irrelevante num app Android puro, então fica fora do build.
...
static const struct VulkanFunc vkDispatchTable[] = {
    ...
    {"vkCreateAndroidSurfaceKHR", vt_call_vkCreateAndroidSurfaceKHR},
