...
void SDLInputDriver::OnWindowAvailable(rex::ui::Window* window) {
  if (window && !attached_window_) {
    ...
    window->app_context().CallInUIThreadSynchronous([this]() {
      ...
      // PORT-FIX(restuff-android): gamepads attached BEFORE this event watch
      // was registered never reach OnControllerDeviceAddedLocked — watchers
      // only fire for events posted after registration (SDL_PushEvent
      // dispatches them at post time, never for events already queued). On
      // Android the virtual pad is attached before the window/input driver
      // exists, so its SDL_EVENT_GAMEPAD_ADDED was consumed by the main loop
      // while controllers_ stayed empty and the guest saw XInput "device not
      // connected" forever (Press START unresponsive). Re-synthesize the
      // ADDED event for every gamepad that is already connected; devices
      // attached later still arrive through the normal watch path.
      {
        int already_connected = 0;
        SDL_JoystickID* pads = SDL_GetGamepads(&already_connected);
        if (pads) {
          for (int i = 0; i < already_connected; ++i) {
            SDL_Event added{};
          ...
          if (already_connected > 0) {
            REXLOG_INFO(
                "SDL input driver: re-synthesized GAMEPAD_ADDED for {} already-connected "
                "gamepad(s) attached before the event watch.",
                already_connected);
          }
        }
      }
