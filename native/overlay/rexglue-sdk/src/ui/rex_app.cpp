...
bool ReXApp::ConstructRuntime(const PathConfig& paths) {
  ...
  // ANDROID PORT (overlay do port): game_data_root aceita TRÊS formas —
  // (a) pasta com o conteúdo do jogo (fluxo original, HostPathDevice);
  // (b) IMAGEM DE DISCO GDFX/XDVDFS (.iso) no host — o disco é montado
  //     no lugar pelo Runtime::SetupVfs via DiscImageDevice (modelo
  //     XenDroid: o arquivo roda onde está, sem cópia);
  // (c) no Android, um content:// URI do SAF apontando para essa imagem
  //     (mmap direto no fd do ContentResolver).
  // O stat de (b) é is_regular_file; (c) NÃO pode passar por std::filesystem
  // (não é um caminho do host) — só conferimos o prefixo content:.
  const std::string game_root_string = paths.game_data_root.string();
  bool is_content_uri = false;
#if REX_PLATFORM_ANDROID
  is_content_uri = filesystem::IsAndroidContentUri(game_root_string);
#endif  // REX_PLATFORM_ANDROID
  std::error_code root_ec;
  const bool is_directory = std::filesystem::is_directory(paths.game_data_root, root_ec);
  const bool is_disc_image =
      is_content_uri || std::filesystem::is_regular_file(paths.game_data_root, root_ec);
  if (!is_directory && !is_disc_image) {
    auto msg = fmt::format("--game_data_root does not exist: {}", game_root_string);
    REXLOG_ERROR("{}", msg);
    rex::ShowSimpleMessageBox(rex::SimpleMessageBoxType::Error, msg);
    return false;
  }
  ...
  // Pre-check do entrypoint NO HOST — só faz sentido no fluxo de PASTA
  // (o ReXApp resolve game:\ → <game_data_root>/... no sistema de
  // arquivos do host). Com uma IMAGEM DE DISCO o jogo:\default.xex vive
  // DENTRO da imagem: o DiscImageDevice resolve pelo VFS e o
  // LoadXexImage abaixo já reporta "File not found"/X_STATUS_NO_SUCH_FILE
  // com caixa de erro caso a imagem não contenha o entrypoint (mesma
  // semântica do Xenia/XenDroid ao bootar um ISO sem Default.xex).
  if (is_directory) {
