...
// [SDKMS] (perf/sd695-30fps v2): paint epilogue split. Field evidence
// (log3, moto g34/SD695, heavy gameplay): the FRAMEMS "sdk" bucket -- the
// stretch from the guest-output refresher's return back to the present
// thread -- measures 56-97ms/frame while the SAME bucket reads 0.1ms in
// menus; it is the single largest unknown in the 117.6ms present cycle
// (prep 35 + rec 5 + sdk 76). In kGuestOutputThreadImmediately mode this
// stretch is PaintAndPresent (acquire + blit/effects/UI record + submit +
// present), so this splits it: trk (submission-tracker wait for the oldest
// in-flight paint -- GPU-availability wait), acq (command pool reset/begin +
// vkAcquireNextImageKHR), rec (guest-output blit / effects / UI drawer
// recording), sub (queue submit), prs (vkQueuePresentKHR). RAII scope: every
// exit path accumulates the phases it passed. Logs [SDKMS] every 30 paints
// (FRAMEMS cadence). Atomics because paint may run on the guest-output OR
// the UI thread depending on mode. No behavior change otherwise.
namespace {
struct SdkmsDiag {
...
struct SdkmsScope {
  ...
  uint64_t trk = 0, acq = 0, rec = 0, sub = 0, prs = 0;
  SdkmsScope() : t0(std::chrono::steady_clock::now()), mark(t0) {}
  void Phase(uint64_t SdkmsScope::*slot) {
    const auto now = std::chrono::steady_clock::now();
    this->*slot += uint64_t(
        std::chrono::duration_cast<std::chrono::microseconds>(now - mark).count());
    mark = now;
  }
  ~SdkmsScope() {
    ...
    g_sdkms.sub.fetch_add(sub, std::memory_order_relaxed);
    g_sdkms.prs.fetch_add(prs, std::memory_order_relaxed);
    const uint64_t n = g_sdkms.n.load(std::memory_order_relaxed);
    if (n % 30 == 0) {
      REXLOG_INFO(
          "[SDKMS] 30-paint avg: trk={}us acq={}us rec={}us sub={}us prs={}us tot={}us "
          "(n={})",
          g_sdkms.trk.load(std::memory_order_relaxed) / 30,
          g_sdkms.acq.load(std::memory_order_relaxed) / 30,
          g_sdkms.rec.load(std::memory_order_relaxed) / 30,
          g_sdkms.sub.load(std::memory_order_relaxed) / 30,
          g_sdkms.prs.load(std::memory_order_relaxed) / 30,
          g_sdkms.tot.load(std::memory_order_relaxed) / 30, n);
      g_sdkms.trk.store(0, std::memory_order_relaxed);
      g_sdkms.acq.store(0, std::memory_order_relaxed);
      g_sdkms.rec.store(0, std::memory_order_relaxed);
      g_sdkms.sub.store(0, std::memory_order_relaxed);
      g_sdkms.prs.store(0, std::memory_order_relaxed);
      g_sdkms.tot.store(0, std::memory_order_relaxed);
    }
  }
};
}  // namespace
...
Presenter::PaintResult VulkanPresenter::PaintAndPresentImpl(bool execute_ui_drawers) {
  ...
  // [SDKMS]: mark BEFORE the result switch so failed acquires still count
  // their (potentially blocking) wait time.
  sdkms.Phase(&SdkmsScope::acq);
  ...
  sdkms.Phase(&SdkmsScope::rec);  // [SDKMS]: blit/effects/UI recording done
  ...
  sdkms.Phase(&SdkmsScope::sub);  // [SDKMS]: queue submit done
  ...
  sdkms.Phase(&SdkmsScope::prs);  // [SDKMS]: present done
