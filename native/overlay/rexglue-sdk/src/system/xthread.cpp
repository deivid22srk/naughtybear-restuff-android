...
void XThread::DeliverAPCs() {
  ...
  while (!user_apc_queue.empty(mem) && kthread->apc_disable_count == 0) {
    ...
    // perf/sd695-30fps v2: this trace fired ~200x/s during gameplay (log3
    // field capture: 12,210 lines in 61s) -- each one a formatted line
    // synchronously flushed through the rotating FUSE file sink plus the
    // logcat mirror, on the very threads whose CPU time should be running the
    // emulated title. Demoted to log_noisy (per-APC tracing restored with
    // log_noisy=1); a 1-in-20000 heartbeat keeps the APC path visible in
    // ordinary debug logs.
    {
      static std::atomic<uint64_t> s_apc_trace{0};
      if (++s_apc_trace % 20000 == 1) {
        REXSYS_INFO("APC: delivery trace suppressed ({} deliveries; log_noisy=1 traces each)",
                    s_apc_trace.load(std::memory_order_relaxed));
      }
    }
    REXSYS_NOISY_DEBUG("Completed delivery of APC to {:08X} ({:08X}, {:08X}, {:08X})", normal_routine,
