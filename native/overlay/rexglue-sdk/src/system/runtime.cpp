...
bool Runtime::SetupVfs() {
  ...
  // ANDROID PORT (overlay do port): game_data_root também pode ser uma
  // IMAGEM DE DISCO (GDFX/XDVDFS) — arquivo .iso no host ou, no Android,
  // um content:// URI do SAF. Nesse caso o disco é MONTADO NO LUGAR via
  // DiscImageDevice (mmap sobre o arquivo/fd do ContentResolver — mesmo
  // modelo do XenDroid): nada é copiado para o armazenamento do app.
  // O Verify() do device checa a assinatura "MICROSOFT*XBOX*MEDIA" nos
  // offsets padrão do XGD (inclui layer de vídeo 0xFD90000) — um arquivo
  // comum falha em Initialize() com erro claro no log.
  const std::string game_root_string = game_data_root_.string();
  ...
  const bool is_disc_image =
      is_content_uri || std::filesystem::is_regular_file(game_data_root_, image_ec);

  const char* mount_path = "\\Device\\Harddisk0\\Partition1";

  if (is_disc_image) {
    // Disco no lugar (ISO selecionado pelo usuário — zero cópia).
    auto device =
        std::make_unique<rex::filesystem::DiscImageDevice>(mount_path, game_data_root_);
    if (!device->Initialize()) {
      REXSYS_ERROR("Runtime::SetupVfs: Failed to initialize disc image device for {}",
                   game_root_string);
      return false;
    }
    const uint64_t files = device->file_count();
    const uint64_t total = device->total_file_size();
    if (!file_system_->RegisterDevice(std::move(device))) {
      REXSYS_ERROR("Runtime::SetupVfs: Failed to register disc image device");
      return false;
    }
    REXSYS_INFO(
        "  Mounted disc image {} at {} (in-place, {} files, {} bytes on disc)", game_root_string,
        mount_path, files, total);

    // game:/d: apontam para o disco montado (mesma semântica da pasta).
    file_system_->RegisterSymbolicLink("game:", mount_path);
    file_system_->RegisterSymbolicLink("d:", mount_path);
    REXSYS_DEBUG("  Registered symbolic links: game:, d:");
  } else {
    // Fluxo original: pasta com o conteúdo do jogo no host.
