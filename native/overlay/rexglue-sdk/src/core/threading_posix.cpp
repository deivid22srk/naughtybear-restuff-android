...
// Port Android: bionic não tem mutexes robustos (PTHREAD_MUTEX_ROBUST /
// pthread_mutex_consistent não existem no NDK). Com o macro indefinido, os
// blocos abaixo caem nos caminhos de mutex normal já presentes no arquivo.
#if REX_PLATFORM_LINUX && !defined(__ANDROID__) && defined(PTHREAD_MUTEX_ROBUST)
