...
std::filesystem::path GetExecutableFolder() {
  ...
  // Port Android (naughtybear-restuff-android): /proc/self/exe resolve para o
  // app_process do sistema (/system/bin/app_process64) — filesystem
  // somente-leitura. O análogo correto da "pasta do executável" é a pasta de
  // arquivos do app (writable), informada pelo launcher via env
  // REX_ANDROID_FILES_DIR antes de qualquer uso (main do JNI). Sem isso,
  // ReXApp::SetupEnvironment tentava create_directories("/system/bin/logs")
  // e abortava com filesystem_error (SIGABRT no SDLThread).
  // Fallback: deriva a pasta do pacote em /proc/self/cmdline.
  if (auto files_dir = rex::platform::env::get("REX_ANDROID_FILES_DIR");
      files_dir && !files_dir->empty()) {
    return std::filesystem::path(*files_dir);
  }
  char cmdline[256] = "";
  if (FILE* f = fopen("/proc/self/cmdline", "rb")) {
    ...
    fclose(f);
    if (n > 0 && cmdline[0] != '\0') {
      return std::filesystem::path("/data/data") / std::string(cmdline) / "files";
    }
  }
...
#if defined(__ANDROID__)
  // Port Android: HOME/XDG_DATA_HOME não existem de forma confiável em
  // app_process; sem isso o fallback cairia em "/.local/share" (read-only →
  // throw em qualquer escrita). Usa a mesma pasta de arquivos do app.
  if (auto files_dir = rex::platform::env::get("REX_ANDROID_FILES_DIR");
      files_dir && !files_dir->empty()) {
    return std::filesystem::path(*files_dir);
  }
