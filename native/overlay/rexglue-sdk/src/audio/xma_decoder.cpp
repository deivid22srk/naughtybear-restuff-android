...
void XmaDecoder::WriteRegister(uint32_t addr, uint32_t value) {
  ...
  if (r >= XmaRegister::Context0Kick && r <= XmaRegister::Context9Kick) {
    ...
    switch (r) {
      case 0x601: {
        // perf/sd695-30fps v2: this is the hottest MMIO write in the game --
        // the guest audio thread toggles the lock-protocol bit ~700x/s in
        // gameplay (log3 field capture: 36,076 lines in 61s across two APU
        // threads). Each write was formatted + flushed through the rotating
        // FUSE file sink (~0.5-1ms syscall per line) and mirrored to logcat,
        // burning ~0.5-1.5 s/s of host CPU and stalling every other thread
        // that tried to log through the same sink mutex. The register has no
        // decoder state attached (see the comment above), so the write is
        ...
        if (++s_lock_suppressed % 100000 == 1) {
          REXAPU_INFO(
              "XMA: lock-protocol write (0601) accepted as no-op ({} writes suppressed; "
              "log_noisy=1 traces each)",
              s_lock_suppressed.load(std::memory_order_relaxed));
        }
        REXAPU_NOISY_DEBUG("XMA: lock-protocol write (0601): {:08X}", value);
        break;
      }
      default: {
        const auto register_info = register_file_.GetRegisterInfo(r);
        if (register_info) {
          REXAPU_NOISY_DEBUG("XMA: Write to unhandled register ({:04X}, {}): {:08X}", r,
                             register_info->name, value);
        } else {
          REXAPU_NOISY_DEBUG("XMA: Write to unknown register ({:04X}): {:08X}", r, value);
        }
        break;
      }
