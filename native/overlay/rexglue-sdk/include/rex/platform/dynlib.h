...
class DynamicLibrary {
 public:
  ...
  // Port Android (naughtybear-restuff-android): assume um handle dlopen já
  // existente (ex.: devolvido pelo adrenotools_open_libvulkan — uma instância
  // única do loader do sistema com hooks de driver customizado). O handle
  // passa a ser destruído pelo Close()/destrutor (dlclose). Não chamar sobre
  // handles que não sejam dono.
  ...
  // Port Android: texto do último dlerror() capturado numa falha de Load
  // (vazio quando não houve falha ou quando indisponível) — para log de
  // diagnóstico real em vez de "dlopen falhou" seco.
