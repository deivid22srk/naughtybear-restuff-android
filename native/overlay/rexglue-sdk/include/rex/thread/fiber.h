...
struct Fiber {
 ...
 private:
  ...
  // bionic não provê a API ucontext (getcontext/makecontext/swapcontext).
  // O contexto é um bloco de registradores callee-saved salvo/restaurado
  // por rex_fiber_switch (assembly arm64 em src/core/fiber_android.cpp).
  struct FiberContext {
    // x19..x28 (índices 0-9), x29/FP (10), x30/LR (11), sp (12)
    uint64_t x[13];
    // d8..d15 — callee-saved SIMD (começa no offset 104 do bloco)
    uint64_t d[8];
  };
  static_assert(sizeof(FiberContext) == 168,
                "layout deve casar com rex_fiber_switch (fiber_android.cpp)");
